...
NumDeviants = 20; %How many deviants? Round up to nearest NumTrials/10. Max possible trials will be NumDeviants *10. 
% Variable ActualNumTrials  = the actual number of trials varying between NumDeviants *5 and NumDeviants *10 to provide randomisation of
% train length between deviants.
...
isStandard = ones(MaxNumTrials,1) % number of standards
...
% %%%% Randomise %%%%
isDeviant = repmat(Shuffle([1:5]),1,NumDeviants) % add 5 possible extra standards before deviant

k=5;
for i = 1:length(isDeviant)
       
    isStandard(k+isDeviant(i)) = 2;
    
    k = k + isDeviant(i) + 5;
    
end

FindTheLastDeviant = find(isStandard==2,NumDeviants); % find the minimum number of trials to give NumDeviants
ActualNumTrials = FindTheLastDeviant(end);

isTrial = isStandard(1:ActualNumTrials);
